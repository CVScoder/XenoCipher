...
int main() {
    ...
    ([&db, &masterKey](const crow::request& req) {
        ...
        try {
            ...
            // Parse health data
            std::regex regex_pattern("HR-(\\d+) SPO2-(\\d+) IR-(\\d+)");
            std::smatch match;
            ...
            int IR = std::stoi(match[3]);

            std::cout << "Health data received - HR: " << heart_rate << ", SPO2: " << spo2 << ", IR: " << IR << std::endl;

            pqxx::work txn(db);
            txn.exec_prepared("insert_health_data", heart_rate, spo2, IR);
            txn.commit();
    ...
    try {
        // Ensure required tables exist
        {
            ...
            ddl.exec("ALTER TABLE health_data\n"
                     ...
                     "  ADD COLUMN IF NOT EXISTS IR integer");
        ...
        prep.exec("PREPARE insert_health_data (smallint, smallint, integer) AS INSERT INTO health_data (heart_rate, spo2, IR) VALUES ($1, $2, $3)");
        prep.commit();
